...
public abstract class SequentialProofMacro extends AbstractProofMacro {
    ...
    /// This array is created on demand using [#createProofMacroArray()].
    ...
    /// {@inheritDoc}
    ///
    /// This launches the first macro and registers a new [AutoModeListener] with the
    /// `mediator`. This listener unregisters itself after the last macro.
    ///
        ...
        for (final ProofMacro macro : getProofMacros()) {
            ...
            for (Node initNode : initNodes) {
                if (macro.canApplyTo(initNode, posInOcc)) {
                    ...
                    pml.taskStarted(new DefaultTaskStartedInfo(TaskStartedInfo.TaskKind.Macro,
                        macro.getName(), 0));
                    synchronized (macro) {
